...
class ScenarioCollection:
    ...
    def get(self, scenario_name, leak_config, sensor_config, sensorfault_config):
        ...
        :returns: A dictionary of measurement data frames, or None if the measuremantes do not exist within the collection
        """
        if scenario_name in self.list_scenarios():
...
class Scenario:
    ...
    def get(self, leak_config, sensor_config_name, sensorfault_config_name):
        ...
        # Read Sensor Config
        sensor_config = ET.parse(os.path.join(self.path, 'sensors', f'{sensor_config_name}.xml')).getroot()
        mask = {}
        ...
        sensorfault_config = ET.parse(os.path.join(self.path, 'sensorfaults', f'{sensorfault_config_name}.xml')).getroot()
        ...
        for config in sensorfault_config:
            ...
            # Make sure that the faul parameter is not None, except for stuckzero faults
            ...
            data[sensor_type].loc[:,part_id] = apply_sensorfault(data[sensor_type].loc[:,part_id], start, end, fault_type, fault_param, rng)

        return data


def apply_sensorfault(df, start, end, fault_type, fault_param, rng):
    """
