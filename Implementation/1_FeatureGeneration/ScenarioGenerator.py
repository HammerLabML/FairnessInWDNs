...
class ScenarioGenerator:
    ...
    def run(self):
        """
        Run the Scenario Generator. This writes all neccesary configs and
    ...
    def read_config(self):
        ...
        xml_schema_doc = lxml.etree.parse('config_schema.xsd')
                ...
                if config_collection.tag == 'SensorConfigs':
                    for sensor_config in config_collection:
                        ...
                        for sensor_collection in sensor_config:
                            # Fill up the entry with the sensors from the correspondng type
    ...
    def create_job_list_and_write_configs(self):
        """
        Iterates through the entire config and (re)writes all neccesary config
        ...
        # Keep track of lowest estimated memroy size
        ...
        for scenario in self.scenarios:
            ...
            if estimated_memory > self.available_memory:
                print(f'[ERROR] Estimated memory for scenario "{scenario_name}" exceeds the available memroy. ({estimated_memory}kB > {self.available_memory}kB) Skipping...')
                continue
        ...
        # Clamp number of threads according to memory constaints
        ...
        if practical_n_threads < self.n_threads:
            ...
            print(f'Clamping threads to {self.n_threads} due to memroy constraints...')
    ...
    def validate_wn(self, wn, network_path):
        ...
        if len(illegal_names) > 0:
            print(f'[WARNING] Some links in {network_path} are named like "leak_segment_*", this name might lead to unexpected behaviour.')

        # Check for illegal node names
        illegal_names = [name for name in wn.nodes if name.startswith('leak_node_')]
        if len(illegal_names) > 0:
            print(f'[WARNING] Some nodes in {network_path} are named like "leak_node_*", this name might lead to unexpected behaviour.')
...
class SimulationJob:
    ...
    def insert_leak(self, leak_info):
        ...
        # Check if peak time is given for incpient leaks
        ...
        if 'nodeId' in leak_info.keys():
            ...
            # Incipient leaks are modeled using pressure dependend demand
            # To not mess up the pressure independend demand of the node, 
        ...
        if leak_type == 'incipient':
            ...
            # this forumla for pressure dependend demand
            ...
            if self.wn.options.time.pattern_timestep != self.time_step:
                ...
                streched_max = self.iterations * self.wn.options.time.pattern_timestep
                pattern = np.interp(np.arange(0, streched_max, scaling_factor), np.arange(0, pattern.shape[0]), pattern)
...
class Worker:
    ...
    def init_worker(available_memory_param, condition_param):
        ...
        :param available_memory_param: multiprocessing.Value memory available for simualtion jobs
        :param condition_param: wait condition for sufficent memory
        """
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(prog='TODO pyhon ScenarioGenerator.py', description='TODO Optional app description', epilog='TODO For docmentation see GITHUB LINK.')
